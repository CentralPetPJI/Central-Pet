...
import { PetRecord, PetResponseRecord } from './models/pet-record';
...
export class PetsService {
  ...
  private async getResponsibleUserLocation(userId: string): Promise<{ city: string; state: string }> {
  ...
  private async enrichPetWithLocation(pet: PetRecord): Promise<PetResponseRecord> {
    const location = await this.getResponsibleUserLocation(pet.responsibleUserId);
    return { ...pet, ...location };
  }
  ...
  async create(dto: CreatePetDto, responsibleUserId: string) {
    ...
    const enriched = await this.enrichPetWithLocation(domain);
    return { message: 'Pet created successfully', data: enriched };
  }
  ...
  async findAll(responsibleUserId?: string) {
    ...
    const enrichedPets = await Promise.all(
      pets.map(async (p) => {
        const domain = PetMapper.toDomain(p as Pet);
        return this.enrichPetWithLocation(domain);
      }),
    );
  ...
  async findOne(id: string) {
    ...
    const domain = PetMapper.toDomain(pet as Pet);
    const enriched = await this.enrichPetWithLocation(domain);
    return { message: 'Pet retrieved successfully', data: enriched };
  }
  ...
  async findByIdForAdoption(id: string): Promise<PetForAdoptionRequest | null> {
    ...
    return {
      ...
      responsibleUserId: enriched.responsibleUserId,
      sourceType: enriched.sourceType,
      sourceName: enriched.sourceName,
      adoptionStatus: enriched.adoptionStatus,
    };
  }
  ...
  async finalizeAdoption(id: string, newResponsibleUserId: string) {
    ...
    const domain = PetMapper.toDomain(updated as Pet);
    const enriched = await this.enrichPetWithLocation(domain);
    return {
      pet: enriched,
  ...
  async update(id: string, dto: UpdatePetDto) {
    ...
    const domain = PetMapper.toDomain(updated as Pet);
    const enriched = await this.enrichPetWithLocation(domain);
    return { message: 'Pet updated successfully', data: enriched };
  }
  ...
  async remove(id: string) {
    ...
    const domain = PetMapper.toDomain(pet as Pet);
    const enriched = await this.enrichPetWithLocation(domain);
    return { message: 'Pet deleted successfully', data: enriched };
  }
}
