...
describe('PetsService', () => {
  ...
  it('deve criar um pet com sucesso', async () => {
    ...
    expect(result.data.age).toBe('ADULTO');
  ...
  it('deve rejeitar criacao com responsibleUserId inexistente', async () => {
  ...
  it('deve rejeitar faixa etaria fora da lista permitida na criacao', async () => {
    await expect(
      validateCreateDto({
        ...makeCreateDto(),
        age: '2 anos',
  ...
  it('deve listar pets filtrando por responsavel', async () => {
  ...
  it('deve rejeitar faixa etaria invalida na atualizacao', async () => {
    await expect(validateUpdateDto({ age: '1 ano' })).rejects.toThrow(BadRequestException);
  });

  it('deve retornar null para adocao quando pet nao tiver responsavel', async () => {
    records.push({
      id: 'pet-no-owner',
      profilePhoto: '',
      galleryPhotosJson: '[]',
  ...
  it('deve finalizar adocao transferindo responsavel e status', async () => {
    const created = await service.create(await validateCreateDto(makeCreateDto()));
  ...
  it('deve realizar soft delete e ocultar pet das consultas', async () => {
    const created = await service.create(await validateCreateDto(makeCreateDto()));
    const requesterId = created.data.responsibleUserId;
