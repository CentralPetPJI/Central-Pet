...
export default function AdoptionRequestsPage() {
  ...
  const requests = activeTab === 'received' ? received.requests : sent.requests;
    ...
    <section className="w-full px-1 pb-8 pt-4 lg:px-0 lg:pt-5">
      ...
      {!isLoading && requests.length > 0 ? (
        <div className="grid gap-4">
          {activeTab === 'received'
            ? (requests as typeof received.requests).map((request) => (
                <AdoptionRequestCard
                  key={request.id}
                  request={request}
            ...
            : (requests as typeof sent.requests).map((request) => (
